        ...
        <div 
          ...
          class="flex justify-center rounded-lg m-4 p-2 shadow focus:rounded-lg focus:shadow-outline focus:border-ssblue text-white border-2 font-semibold sm:w-1/3 lg:h-sm">
            <g-link :to="`/guns/${result.slug}`">
              <figure :v-if="result.images[0].FullPath">
                <g-image :src="`${result.images[0].FullPath}`" class="rounded"></g-image>
                <h2 class="hover:text-ssorange hover:border-ssorange mt-4 text-md" v-if="result.variant != null">{{ result.condition + ' ' + result.title + ' ' + result.variant }}</h2>
                <h2 class="hover:text-ssorange hover:border-ssorange" v-else>{{ result.condition + ' ' + result.title }}</h2>
                <p>£{{result.price}}</p>
              </figure>
            </g-link>
        </div>
